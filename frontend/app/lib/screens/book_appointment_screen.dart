...
class _BookAppointmentScreenState extends State<BookAppointmentScreen> {
  ...
  Future<void> _openUpiApp(String method, String? upiId) async {
    ...
    final paymentAmount = PaymentService.calculateAppointmentFee(_selectedHospital!.id);
    ...
    switch (method) {
      case 'googlepay':
        upiUrl = 'tez://upi/pay?pa=$encodedUpiId&pn=$encodedHospitalName&am=$encodedAmount&cu=INR&mc=0000';
        break;
      case 'phonepe':
        upiUrl = 'phonepe://pay?pa=$encodedUpiId&pn=$encodedHospitalName&am=$encodedAmount&cu=INR&mc=0000';
        break;
      case 'paytm':
        upiUrl = 'paytmmp://pay?pa=$encodedUpiId&pn=$encodedHospitalName&am=$encodedAmount&cu=INR&mc=0000';
        break;
      case 'bhim':
        upiUrl = 'upi://pay?pa=$encodedUpiId&pn=$encodedHospitalName&am=$encodedAmount&cu=INR&mc=0000';
        break;
      default:
        upiUrl = 'upi://pay?pa=$encodedUpiId&pn=$encodedHospitalName&am=$encodedAmount&cu=INR&mc=0000';
        break;
    ...
    try {
      ...
      final paymentOrderResponse = await PaymentService.createPaymentOrder(
        ...
        metadata: {
          ...
          'appointment_time': '${_timeController.text.trim()} $_selectedAmPm',
  ...
  Future<void> _confirmPaymentAndBook() async {
    ...
    try {
      ...
      // In production, you'd verify the actual payment from UPI gateway
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        try {
          ...
          final errorMsg = error['detail'] ?? 'Failed to book appointment';
          Fluttertoast.showToast(
            msg: errorMsg,
            backgroundColor: AppColors.errorColor,
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isLoading
                      ...
                      : Text(
                          _showPaymentOptions ? 'Confirm Payment & Book Appointment' : 'Proceed to Payment',
                          style: const TextStyle(
                            fontSize: 18,
                            fontWeight: FontWeight.bold,
                            color: Colors.white,
