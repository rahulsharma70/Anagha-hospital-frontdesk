...
def send_daily_reminders():
    ...
    try:
        ...
        operations = supabase.table("operations").select("*").eq("date", today).eq("status", "confirmed").execute()
        ...
        if pending.data:
            ...
            for msg in pending.data:
                try:
                    ...
                    supabase.table("whatsapp_logs").update({"status": "failed", "error": str(e)}).eq("id", msg["id"]).execute()
    except Exception as e:
